                    ...
                    uint16_t length)
{
  ...
  zdo_msg_report(sender, apsFrame, message, length);
...
//                       apsFrame->clusterId);
//   emberAfDebugFlush();
	emberAfAppPrintln("Processing message: len=%d profile=%2x cluster=%2x",
                      messageLength,
                      apsFrame->profileId,
                      apsFrame->clusterId);
...
void emAfMessageSentHandler(EmberOutgoingMessageType type,
                            ...
                            uint8_t messageTag)
{
	printf("emAfMessageSentHandler\n");
  EmberAfMessageSentFunction callback;
  if (status != EMBER_SUCCESS) {
    emberAfAppPrint("%ptx %x, ", "ERROR: ", status);
    printMessage(type, apsFrame, messageLength, messageContents);
  }
	emberAfAppPrint("type: 0x%02x, indexOrDestination: 0x04x, messageTag: 0x%02x, status: 0x%02x\n", type, indexOrDestination, messageTag, status);
	emberAfAppPrint("messageLength: %d\n", messageLength);
	int i = 0;
	for(;i < messageLength; i++)
	{
		emberAfAppPrint("0x%02x ", messageContents[i]);
	}
	emberAfAppPrint("\n");
  ...
  for (i = 0; i < EMBER_SUPPORTED_NETWORKS; i++) {
    ...
    if (emAfProIsCurrentNetwork()) {
      ...
      if (emAfCurrentZigbeeProNetwork->nodeType == EMBER_COORDINATOR) {
		  emberAfAppPrintln("emAfNetworkInit->zaTrustCenterSecurityPolicyInit");
