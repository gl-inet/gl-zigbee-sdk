    ...
    uint8_t stopBits)
{
    ...
    printf("emberSerialInit()\n");
    if (port > 1) {
        return EMBER_SERIAL_INVALID_PORT;
    }

    if (childPid[port] != INVALID_PID) {
        printf("Serial port %d already initialized.\n", port);
    ...
    if (backchannelEnable) {
        ...
        BackchannelState state = backchannelCheckConnection(port,
...
static void setCurrentCommandPtr(const char* nextToken)
{
    ...
    while (subMenuPtr != NULL && subMenuPtr[i].name != NULL) {
        ...
        if (0 == strncmp(subMenuPtr[i].name, wordStart, strlen(subMenuPtr[i].name))
            && *wordEnd == ' ') {
