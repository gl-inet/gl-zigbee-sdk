...
//------------------------------------------------------------------------------
// Functions
void get_module_mac(char *mac)
{
	sprintf(mac,"%02x:%02x:%02x:%02x:%02x:%02x:%02x:%02x", \
				emLocalEui64[7], emLocalEui64[6], emLocalEui64[5], emLocalEui64[4], \
				emLocalEui64[3], emLocalEui64[2], emLocalEui64[1], emLocalEui64[0] );
	return ;
}
...
int emberAfMain(int argc, char* argv[])
{
    //Initialize the hal
    halInit();
...
// *                  USER CREATE									 *
// *******************************************************************
int emberAfMainStart(void)
{
    //Initialize the hal
    halInit();
    INTERRUPTS_ON(); // Safe to enable interrupts at this point

    printf("Reset info: %d (%s)\n", halGetResetInfo(), halGetResetString());
...
{
    emberAfAppPrintln("ezspIncomingMessageHandler");
...
void ezspPrintfHandler(uint16_t ID)
{
    emberAfCorePrintln("Frame IDs: 0x%04x", ID);
    emberAfCoreFlush();
}
